...
@FluentConfiguration(webDriver = "chrome", baseUrl = "http://localhost:8080")
@Wait
...
//		webDriver.manage().window().setSize(new Dimension(1440,960));
	...
	public void deleteSession() {
		loginPage.go();
		loginPage.fillAndSubmitForm("thiago", "admin");
		loginPage.isAt();

		executeScript("PF('sessionDate').setDate(new Date(2017,0,1));");
		executeScript("PF('sessionDate').fireDateSelectEvent();");
		executeScript("PF('sessionTime').setTime('08:00');");

		for (int i = 0; i < 5; i++) {
			executeScript("PF('sessionPatients').check($(PF('sessionPatients').checkboxes[" + i + "])); $(PF('sessionPatients').inputs[" + i + "]).prop('checked', true);");
		}
